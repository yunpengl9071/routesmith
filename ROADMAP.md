...
| Feature scaling (context sensitivity) | ❌ **Raw-scale features drown context signals — bandit is nearly context-blind** (measured 2026-07-06; fixed by Task P1.0) |
| Metrics export / auth / multi-replica state | ❌ None |
| CI | ❌ **No workflows exist** (added by Task P0.0) |
...
| G4 | Learning convergence | ≥ 80% optimal-arm selection within 150 feedback events (synthetic 2-arm workload, seeded) — **requires Task P1.0**; see Validation notes | `pytest tests/test_convergence.py` passes |
...
## Validation notes (2026-07-06, commit `a1e5e5c`)

The quantitative targets were validated empirically before being baked into tests —
executors should treat these as ground truth, not aspirations:

- **G3 (routing overhead):** measured `router.route()` with LinTS + 5 models over 200
  iterations: **p50 = 0.97 ms, p99 = 1.67 ms**. The 5 ms / 15 ms thresholds have ~5×
  headroom; a failure indicates a real regression.
- **G4 (convergence):** the exact G4 harness was prototyped against the real
  `LinTSPredictor`. On CURRENT code: 45–59/100 across seeds {42, 0, 1, 7, 123} — the
  bandit is nearly context-blind because raw-scale features (latency_p50 = 500, char counts
  in the hundreds) dominate the L2-normalized vector and drown the [0,1]-scale
  keyword/context signals; it degenerates to per-arm average reward (≈ always pick the
  on-average-best model). With Task P1.0's per-feature scale vector applied: **88–95/100
  on the same seeds**. `log1p` squashing alone was tested and is NOT sufficient (57–73/100).
  Conclusion: P1.0 is a hard prerequisite for G4 and for the "contextual" claim generally.
...
| 1 | [docs/plans/phase-1-learning-loop.md](docs/plans/phase-1-learning-loop.md) | Close the learning loop: feature normalization (P1.0), proxy feedback, implicit signals, LLM judge | G4, G6 |
...
5. **Mock `litellm` in unit tests.** Never make real API calls in tests. Pattern:
   ...
   If `import litellm` itself fails in your environment (native cffi/cryptography deps),
   export `RS_TEST_MOCK_LITELLM=1` — Task P0.0 adds a conftest shim that stubs the module
   before `routesmith` imports it. CI does not set this variable.
