...
\item
  ...
  keywords or metadata (e.g., ``billing'' cheaper model). This works for
  predictable patterns but fails on ambiguous queries.
\item
...
\item
  ...
  interpretable diagnostics a novelty over global priors in prior bandit
  formulations.
\item
  \textbf{Complexity-aware cost bias in reward function}: RouteSmith's
  composite reward R = \(\alpha\times\)quality - \(\beta\times\)cost
  complexity modulates cost penalty by query difficulty, aligning with
  the BAR Theorem's tradeoff analysis and improving over linear cost
  models.
\item
...
regularization (Farahmand \& Szepesv ri, 2010), with algorithms like
...
\textbf{RouteSmith (Ours)} & \textbf{Thompson Sampling} & ** ** & ** **
& \textbf{Complexity-aware} & \textbf{Comprehensive} \\
\bottomrule
\end{longtable}
...
\midrule
\endhead
Mean Cost/Query & \$1.991 \(\pm\) \$0.121 & \$0.476 \(\pm\) \$0.026 &
75.99\% \\
...
\textbf{Baselines:} 1. \textbf{Static Premium:} All queries premium tier
(\$0.0228/query) 2. \textbf{Category Mapping:} Fixed routing (technical
premium, FAQ economy)
...
95\% CI (Wilson score): {[}96.4\%, 100\%{]} exceeds production threshold
(95\%).
...
\textbf{Production-ready} 100\% success rate, no infrastructure issues
3. \textbf{Free models viable} 37\% of queries handled by free Nemotron
4. \textbf{Cost control proven} \$1.44 for 100 queries is
production-viable
