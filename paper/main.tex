...
\noindent \textbf{Results.} On binary routing (GPT-4o vs.\ GPT-4o-mini, 600 MMLU + 300 GSM8K questions with real API calls), LinTS-27d achieves APGR=0.593 with 46\% MMLU cost reduction versus Always-Strong, while LinUCB-27d achieves APGR=1.126 by selective strong-arm routing (but APGR$\approx$0 on GSM8K, where aggressive routing wastes cost). On 5-arm multi-model routing (GPT-4o, Claude-Sonnet-4.5, Qwen-Plus, MiniMax-M1, DeepSeek-V3)---structurally impossible for binary routers---LinTS achieves 71.0\% accuracy with 45\% cost savings, converging to a stable policy across seeds. Online bandits learn from $\sim$100 queries, compared to 55K+ labels required by supervised approaches.
...
We present \cbroutesmith{}, which applies two contextual bandit algorithms over 27 query features:
\begin{itemize}
...
\end{equation}
where $\alpha_q + \alpha_c = 1$, $c_{a_t}$ is the per-query cost of arm $a_t$, and $c_{\max}$ is a fixed cost normalization (300 input + 50 output tokens on the strong model). For Experiment 1 (binary routing), $\alpha_q = 1.0$, $\alpha_c = 0.15$ (quality weight omitted for simplicity; the 0.15 cost penalty alone produced sufficient cost-quality separation). For Experiment 2, $\alpha_q = 0.85$, $\alpha_c = 0.15$.
...
\begin{itemize}
    ...
    \item \textbf{Model features [17--26]:} Zero-padded in the benchmark (arm distinction is via arm index, not features); in production, these encode per-model metadata including cost rates, quality priors, latency, and capability flags~\cite{routesmith2025system}.
\end{itemize}
...
\end{align}
The UCB score $p_a(x_t)$ trades off estimated quality (first term) with uncertainty (second term, controlled by $\beta$). The benchmark implementation uses naive $O(d^3)$ matrix inversion ($d=27$, negligible on modern hardware); the production RouteSmith system uses Sherman-Morrison rank-1 updates ($O(d^2)$) for the same computation~\cite{routesmith2025system}.
...
\textbf{Key advantage over LinUCB:} LinTS has no $\beta$ exploration bonus hyperparameter. The only knob is $v^2$ (posterior variance scaling, fixed at $v^2=1.0$), which controls the width of the posterior samples but requires no threshold tuning. Exploration is proportional to posterior variance, which decays naturally as data accumulates. LinTS achieves $O(d\sqrt{T \log T})$ cumulative regret~\cite{abeille2017linear}, matching LinUCB's order with essentially no manual tuning.
...
\end{equation}
Route to strong if $\theta_k > 0.5$. Unlike LinUCB/LinTS, TS-Cat does not use the continuous reward function. Instead, it uses a binary quality-only update: if the weak model answers correctly, $\beta_k$ is incremented (weak was sufficient); otherwise $\alpha_k$ is incremented (strong was needed). There is no cost signal in the update. TS-Cat captures category-level routing preferences but ignores query-level features.
...
We evaluate all methods on 600 MMLU questions (5 categories $\times$ 120) and 300 GSM8K math word problems. Models: strong = \texttt{openai/gpt-4o}, weak = \texttt{openai/gpt-4o-mini} (via OpenRouter). Bandit methods run with 5 independent random seeds; mean $\pm$ 1$\sigma$ reported. Random router uses a single seed (42). RouteLLM-SW is deterministic per threshold---point estimate with its pretrained model pair (GPT-4-1106 vs.\ Mixtral-8x7b) differing from our evaluation pair. We do not perform formal significance tests (e.g., paired bootstrap, McNemar) given the limited number of seeds; standard deviations are reported for descriptive purposes only.

Reward: $R_t = \text{acc}(a_t, x_t) - 0.15 \cdot c_{a_t} / c_{\max}$.

\paragraph{Always-call-weak design.} All bandit strategies (LinTS, LinUCB, TS-Cat) call the weak model on every query to obtain the reward signal needed for online learning. When routed to strong, the strong model is called in addition, making this a cascade design. Reported costs reflect this: a query routed to strong incurs both model costs. This adds overhead versus a production deployment that would call only the selected model, but the relative comparison between strategies remains valid.

\subsubsection{Results}

\begin{table}[h]
\centering
\caption{Experiment 1: Binary routing comparison on MMLU (600 questions) and GSM8K (300 questions). APGR is computed on MMLU. Bandit methods: 5 seeds, mean $\pm$ 1$\sigma$. Random: single seed (42). RouteLLM-SW: deterministic per threshold. All results from real API calls via OpenRouter.}
\label{tab:exp1}
...
LinUCB-27d achieves the highest MMLU accuracy (78.2\%) and APGR (1.126), exceeding Static-Strong by routing selectively. On GSM8K, however, LinUCB's aggressive strong-arm routing increases cost (\$0.126) without accuracy improvement (91.7\% vs 91.7\% weak)---APGR is essentially zero. LinTS-27d matches Random on MMLU (75.8\%, APGR=0.593) while achieving higher GSM8K accuracy (92.5\%), reflecting better uncertainty quantification on math problems. The key distinction: LinUCB requires $\beta$ tuning (APGR varies 0.407--0.741 across $\beta \in \{0.5, 1.5, 3.0\}$, see \S\ref{sec:ablations}), while LinTS requires no hyperparameter.

\paragraph{RouteLLM-SW (domain-transfer stress test).}
RouteLLM-SW routes 0\% of queries to strong across all thresholds. Win-rate scores cluster at 0.218--0.233, below all thresholds. This reflects domain mismatch between Chatbot Arena conversational queries and structured MMLU/GSM8K benchmarks. Additionally, the SW router was pretrained on a different model pair (GPT-4-1106 vs.\ Mixtral-8x7b, from the original RouteLLM repository) than our evaluation pair (GPT-4o vs.\ GPT-4o-mini)---part of the domain transfer. \textbf{We emphasize this is a domain-transfer stress test and should not be interpreted as a claim about RouteLLM-SW's native performance on its intended model pair.} Rather, it demonstrates that online bandits learn task-specific routing without relying on pre-calibrated domain embeddings.
...
Five model arms via OpenRouter: GPT-4o (\$2.50/\$10.00), Claude-Sonnet-4.5 (\$3.00/\$15.00), Qwen-Plus (\$0.40/\$1.20), MiniMax-M1 (\$0.30/\$1.10), DeepSeek-V3 (\$0.20/\$0.77). Reward: $R_t = 0.85 \cdot \text{acc}(a_t, x_t) - 0.15 \cdot c_{a_t}/c_{\max}$. Evaluated on 600 MMLU questions with 3 seeds (42, 43, 44).
...
\begin{table}[h]
\centering
\caption{Experiment 2: LinTS-5arm multi-model routing on MMLU (600 questions, 3 seeds).}
\label{tab:exp2}
\small
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{Seed} & \textbf{Acc.} & \textbf{Cost} & \textbf{Top-3 Routing} \\
\midrule
42 & 71.0\% & \$0.104 & GPT-4o 34\%, Qwen-Plus 46\%, DeepSeek 10\% \\
43 & 70.7\% & \$0.131 & GPT-4o 45\%, Qwen-Plus 32\%, DeepSeek 12\% \\
44 & 71.3\% & \$0.117 & GPT-4o 40\%, Qwen-Plus 41\%, DeepSeek 10\% \\
\midrule
\textbf{Mean} & \textbf{71.0$\pm$0.3\%} & \textbf{\$0.117} & \text{GPT-4o $\approx$40\%, Qwen $\approx$40\%, DeepSeek $\approx$11\%} \\
\bottomrule
\end{tabular}
\end{table}
...
We initialize LinTS with 0, 100, or 500 oracle labels (always reward arm~1, the strong model) before online evaluation on the remaining queries (600, 500, 100 respectively). Using subset-corrected baselines (strong/weak accuracy recomputed on the same eval queries to avoid data leakage), APGR improves from 0.759 (cold start, 600 queries) to 0.897 (100 labels, 500 queries) and 0.800 (500 labels, 100 queries). The non-monotonic improvement---100 labels outperforms 500---arises because 500 oracle labels strongly bias the posterior toward the strong arm, reducing exploration on only 100 remaining queries. Even 100 warm-start labels substantially reduce cold-start regret, suggesting that a small labeled calibration set is beneficial when available.
...
LinTS-27d on GSM8K achieves 92.5\%$\pm$0.4\% (APGR=0.153 using strong=97.3\%, weak=91.7\%). This is higher than TS-Cat (91.1\%, APGR=$-0.106$), showing the 27-dim features provide routing signal beyond category-level preferences. The lower APGR on GSM8K reflects the smaller strong/weak gap (5.6pp vs 4.5pp on MMLU).

\subsection{Reproducibility}

All results in this paper are from real API calls with fixed random seeds, ensuring full reproducibility. Experiment scripts, configurations, and random seeds are provided in the supplementary material. Hyperparameters are encoded in result filenames (e.g., \texttt{alpha1.5}, \texttt{vsq1.0}); algorithm state is stored per-row in bandit results.
...
\textbf{Limitations.}
\begin{itemize}
    \item \textbf{Evaluation scope:} Our experiments use 600 MMLU questions (4.3\% of the full 14,042-question benchmark) and 300 GSM8K problems (22.7\% of 1,319). No evaluation covers code generation (MBPP cached but not run), conversational tasks, or real-world assistant workloads.
    \item \textbf{Statistical significance:} We report $\pm$ 1$\sigma$ from 5 seeds descriptively but do not perform formal significance tests (paired bootstrap, McNemar). Where standard deviations overlap (e.g., LinTS at 75.8\% vs.\ Random at 75.8\% on MMLU), the methods are not distinguishable at 5 seeds.
    \item \textbf{RouteLLM-SW comparison:} The SW router was pretrained on GPT-4-1106 vs.\ Mixtral-8x7b and evaluated on GPT-4o vs.\ GPT-4o-mini. This model pair mismatch, combined with domain transfer from conversational to structured benchmarks, prevents drawing strong comparative conclusions about RouteLLM's native performance on its intended pair.
